...
const {
  ...
  `/v2/ads/read-all-active-ads`, //enpoint
  "get", //method
  "ads/read-all-active-ads" //key
)
...
const categoryName = categoryId === "" ? "Value Meal" : getCategoryName(categoryId,result)
  ...
  return (
    <>
      <SliderBanner isLoadingAds={isLoadingAds} isFetchingAds={isFetchingAds} errorAds={errorAds} dataAds={dataAds} />
      ...
      <div className="grid grid-rows-[auto,_1fr,_auto] min-h-[calc(100vh-200px)]">
        ...
        <div className="flex justify-between items-center bg-myred text-white p-1 px-3">
          ...
          >
            {cartData.length > 0 && (
              <span className="absolute -top-2 -left-2 text-[12px] bg-white text-myred rounded-full size-[20px] font-bold grid place-content-center">
                {cartData.length}
              </span>
            )}
            View Cart
